...
if (isset($_SESSION['username']))
{
	...
	////////////////
	// Query Rank //
	$rank_query = 'SELECT `name`, `rank`, `elo_rating`, `compound_rating` FROM `players` WHERE `id_registrations`="'.$_SESSION['userid'].'" ORDER BY `rank` ASC';
	$run_rank_query = mysql_query($rank_query);
	////////////////

	//////////////////
	// Query Record //
	$record_query = 'SELECT `name`, `wins`, `losses`, `ot_losses`, `ot_games_played`, `cup_dif` FROM `players` WHERE `id_registrations`="'.$_SESSION['userid'].'" ORDER BY `wins` DESC';
	$run_record_query = mysql_query($record_query);
	//////////////////

	////////////////////
	// Query Shooting //
	$shooting_query = 'SELECT `name`, `shots`, `hits`, `shooting_percentage`, `bounces`, `gang_bangs`, `errors`, `heating_up`, `on_fire`, `h1` FROM `players` WHERE `id_registrations`="'.$_SESSION['userid'].'" ORDER BY `shooting_percentage` DESC';
	$run_shooting_query = mysql_query($shooting_query);
	////////////////////

	///////////////////
	// Query Streaks //
	$streaks_query = 'SELECT `name`, `win_streak`, `loss_streak`, `hit_streak`, `miss_streak` FROM `players` WHERE `id_registrations`="'.$_SESSION['userid'].'" ORDER BY `hit_streak` DESC';
	$run_streaks_query = mysql_query($streaks_query);
	///////////////////

	//////////////////////
	// Query Redemption //
	$redemption_query = 'SELECT `name`, `redemp_shotperc`, `redemp_shots`, `redemp_hits`, `redemp_atmps`, `redemp_succs` FROM `players` WHERE `id_registrations`="'.$_SESSION['userid'].'" ORDER BY `redemp_shotperc` DESC';
	$run_redemption_query = mysql_query($redemption_query);
	//////////////////////

	/////////////////////////
	// Query Rack Shooting //
	$rack_query = 'SELECT `name`, `p10`, `p9`, `p8`, `p7`, `p6`, `p5`, `p4`, `p3`, `p2`, `p1` FROM `players` WHERE `id_registrations`="'.$_SESSION['userid'].'" ORDER BY `p1` DESC';
...
<body>
	...
	<div id="bottom">
		...
		<div id="bottom_right">
			<h1 class="main_heading">Overall Stats</h1>
			<?php if ($is_logged_in) { ?>
